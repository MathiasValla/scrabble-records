#!/usr/bin/env python3
"""Exhaustive upper-bound search for three-tile English Scrabble moves.

For a move placing exactly three tiles in one row, the formed words are:

* one main horizontal word containing all three new tiles; and
* zero to three vertical cross words, one at each new tile.
...
class MainPlacement:
    ...
    new_cells: tuple[tuple[int, int], tuple[int, int], tuple[int, int]]
...
def main_placement_from_occurrence(
    ...
    if len(indices) != 3:
        raise AssertionError(f"three indices expected, got {indices}")
    new_cells = (cells[indices[0]], cells[indices[1]], cells[indices[2]])
    word_multiplier = 1
...
def record_payload(
    ...
    excess: dict[str, int],
    main: MainPlacement,
    crosses: tuple[CrossOption, CrossOption, CrossOption],
) -> dict[str, object]:
    return {
        "score": score,
...
def evaluate_cross_product(
    main: MainPlacement,
    option_lists: tuple[list[CrossOption], list[CrossOption], list[CrossOption]],
    best: int,
) -> tuple[int, list[tuple[int, int, dict[str, int], tuple[CrossOption, CrossOption, CrossOption]]], int]:
    records: list[tuple[int, int, dict[str, int], tuple[CrossOption, CrossOption, CrossOption]]] = []
    checked = 0
    selected: list[CrossOption] = []
    ...
    if blank_count_for_counts(main_counts) > 2:
        return best, records, checked
    top_from = [0, 0, 0, 0]
    for slot in range(2, -1, -1):
        top_from[slot] = top_from[slot + 1] + option_lists[slot][0].face_score
    ...
    def dfs(
        ...
        if slot == 3:
            ...
            if score is None or score < best:
                return
            crosses = (selected[0], selected[1], selected[2])
            records.append((score, penalty, excess, crosses))
            if score > best:
                best = score
            return

        remaining_after_slot = top_from[slot + 1]
        for option in option_lists[slot]:
            ...
            if blank_count_for_counts(next_counts) > 2:
                continue
            next_entries = entries + option.entries
            selected.append(option)
            dfs(slot + 1, next_face, next_entries, next_counts)
            selected.pop()
...
def search(
    ...
    *,
    ...
    print(f"\n=== {lexicon_name} three-tile search ===")
    ...
    seed_heap: list[tuple[int, int, int, str, tuple[int, int, int], int, int]] = []
    serial = 0
    raw_triples = 0
    kept_triples = 0
    main_placements = 0
    ...
    def evaluate_job(
        ...
        nonlocal best, records, checked_main, checked_cross_triples
        if upper < best:
            return
        checked_main += 1
        ...
        option_lists = (
            get_cross_options(main.letters[0], main.new_cells[0]),
            get_cross_options(main.letters[1], main.new_cells[1]),
            get_cross_options(main.letters[2], main.new_cells[2]),
        )
        previous_best = best
        new_best, hits, checked = evaluate_cross_product(main, option_lists, best)
        checked_cross_triples += checked
                ...
                for k in range(j + 1, length):
                    ...
                    for row in range(1, 16):
                        for start_col in range(1, 17 - length):
                            ...
                            cells = ((row, start_col + i), (row, start_col + j), (row, start_col + k))
                            upper = main_face + sum(cross_face_upper[(word[index], cell)] for index, cell in zip(indices, cells))
                            serial += 1
        ...
        if now - last_report >= 20:
            ...
            if certify_from_initial:
                print(
                    ...
                    f"raw triples {raw_triples:,}; kept {kept_triples:,}; "
                    f"placements {main_placements:,}; checked main {checked_main:,}; "
                    f"checked triples {checked_cross_triples:,}; best {best}; "
    ...
    print(f"main three-hole occurrences: {kept_triples:,} deletion-valid / {raw_triples:,} raw")
    print(f"main placements: {main_placements:,}; seed placements retained: {len(seed_heap):,}")

    if certify_from_initial:
        print(f"checked main placements exactly: {checked_main:,}; checked cross triples: {checked_cross_triples:,}")
        ...
        return {
            ...
            "best_score": best,
            "records": records,
            "raw_main_triples": raw_triples,
            "deletion_valid_main_triples": kept_triples,
            "main_placements": main_placements,
            "checked_main_placements_exactly": checked_main,
            "checked_cross_triples": checked_cross_triples,
...
def main() -> None:
    ...
    out_path = OUT / (
        "three_tile_certify_search.json"
        if args.certify_from_initial
        else "three_tile_seed_search.json"
        if args.seed_only
        else "three_tile_search.json"
    )
