#!/usr/bin/env python3
"""Exhaustive upper-bound search for five-tile English Scrabble moves.

For a move placing exactly five tiles in one row, the formed words are:

* one main horizontal word containing all five new tiles; and
* zero to five vertical cross words, one at each new tile.
...
LETTER_VALUES_TUPLE = tuple(LETTER_VALUES[ch] for ch in ALPHABET)
TILE_DISTRIBUTION_TUPLE = tuple(TILE_DISTRIBUTION[ch] for ch in ALPHABET)
NEW_TILE_COUNT = 5
HOLE_LABEL = "five-hole"
HOLE_TUPLE_LABEL = "quintuples"
...
def blank_count_for_counts(counts: tuple[int, ...]) -> int:
    total = 0
    for count, available in zip(counts, TILE_DISTRIBUTION_TUPLE):
        if count > available:
            total += count - available
    return total


def optimistic_blank_penalty_for_counts(counts: tuple[int, ...]) -> int:
    """A valid lower bound on the blank penalty for a partial scoring pattern."""
    total = 0
    for count, available, value in zip(counts, TILE_DISTRIBUTION_TUPLE, LETTER_VALUES_TUPLE):
        if count > available:
            total += (count - available) * value
    return total
    ...
    *,
    ...
    def dfs(
        ...
        for option in option_lists[slot]:
            ...
            if next_face + remaining_after_slot < best:
                break
            ...
            if next_face + remaining_after_slot - optimistic_blank_penalty_for_counts(next_counts) < best:
                continue
            partial_score, _partial_penalty, _partial_excess = score_after_best_blanks(
                next_face,
                entries + option.entries,
            )
            if partial_score is None or partial_score + remaining_after_slot < best:
                continue
...
def capped_options(options: list[CrossOption], option_cap: int) -> list[CrossOption]:
    if option_cap <= 0 or option_cap >= len(options):
        return options
    capped = list(options[:option_cap])
    if NONE_CROSS not in capped:
        capped.append(NONE_CROSS)
    capped.sort(key=lambda option: option.face_score, reverse=True)
    return capped


def evaluate_cross_product_beam(
    main: MainPlacement,
    option_lists: tuple[list[CrossOption], ...],
    best: int,
    *,
    ...
    """Heuristic lower-bound search over a capped cross-product.

    This is intentionally not used as a proof.  It is a fast way to find a high
    feasible score so that the later exhaustive pass has a sharper threshold.
    """
    records: list[tuple[int, int, dict[str, int], tuple[CrossOption, ...]]] = []
    checked = 0
    nodes = 0
    ...
    capped_lists = tuple(capped_options(options, option_cap) for options in option_lists)
    ordered_slots = tuple(
        sorted(
            range(len(capped_lists)),
            key=lambda slot: (len(capped_lists[slot]), -capped_lists[slot][0].face_score),
        )
    )
    top_from = [0] * (len(ordered_slots) + 1)
    for offset in range(len(ordered_slots) - 1, -1, -1):
        slot = ordered_slots[offset]
        top_from[offset] = top_from[offset + 1] + capped_lists[slot][0].face_score

    empty_selection: tuple[CrossOption | None, ...] = tuple(None for _ in capped_lists)
    states = [(main.face_score, main.entries, main_counts, empty_selection)]
    serial = 0
    ...
    for offset, slot in enumerate(ordered_slots):
        ...
        heap: list[
            tuple[
                ...
                tuple[int, ...],
                tuple[CrossOption | None, ...],
            ]
        ] = []
        for face_score, entries, counts, selected in states:
            for option in capped_lists[slot]:
                nodes += 1
                next_face = face_score + option.face_score
                next_counts = add_count_tuples(counts, option.counts_without_new)
                if blank_count_for_counts(next_counts) > 2:
                ...
                optimistic = (
                    ...
                    - optimistic_blank_penalty_for_counts(next_counts)
                )
                if optimistic < best:
                    continue
                next_selection = list(selected)
                next_selection[slot] = option if option.word is not None else None
                serial += 1
                item = (
                    optimistic,
                    serial,
        ...
        states = [
            (face_score, entries, counts, selected)
            for _, _, face_score, entries, counts, selected in sorted(heap, reverse=True)
        ]
        if not states:
            return best, records, checked, nodes

    for face_score, entries, _counts, selected in states:
        checked += 1
        score, penalty, excess = score_after_best_blanks(face_score, entries)
        if score is None or score < best:
            continue
    ...
    *,
    ...
    def evaluate_job(
        ...
        *,
        ...
        previous_best = best
        if use_beam:
            new_best, hits, checked, nodes = evaluate_cross_product_beam(
                main,
                option_lists,
                best,
                keep_ties=keep_records,
                option_cap=beam_option_cap,
        ...
        if now - last_report >= report_seconds:
            ...
            if estimate_only:
                print(
                    ...
                    f"raw {HOLE_TUPLE_LABEL} {raw_quadruples:,}; kept {kept_quadruples:,}; "
    ...
    print(f"main {HOLE_LABEL} occurrences: {kept_quadruples:,} deletion-valid / {raw_quadruples:,} raw")
    ...
    if estimate_only:
        ...
        return {
            ...
            "raw_main_quintuples": raw_quadruples,
            "deletion_valid_main_quintuples": kept_quadruples,
    ...
    if beam_only:
        for job_no, (upper, _, main_face, word, indices, row, start_col) in enumerate(
            sorted(seed_heap, reverse=True)[:beam_jobs],
            start=1,
        ):
            if job_no == 1 or job_no % 25 == 0:
                print(
                    "beam pass "
                    f"{job_no:,}/{min(beam_jobs, len(seed_heap)):,} retained jobs; "
                    f"best {best}; upper {upper}; word {word}"
                )
            evaluate_job(upper, main_face, word, indices, row, start_col, use_beam=True)
        return {
            "lexicon": lexicon_name,
            "word_count": len(legal_words),
            "best_score_beam_only": best,
            "records": records,
            "raw_main_quintuples": raw_quadruples,
            "deletion_valid_main_quintuples": kept_quadruples,
            "main_placements": main_placements,
            ...
            "beam_jobs": beam_jobs,
            "beam_option_cap": beam_option_cap,
            "beam_width": beam_width,
            "checked_main_placements_by_beam": checked_main,
            "beam_leaf_states_checked": checked_cross_quadruples,
            "beam_nodes": checked_cross_nodes,
            "cross_option_cache_entries": len(cross_cache),
        }
...
def main() -> None:
    ...
    parser.add_argument("--beam-jobs", type=int, default=1_000)
    parser.add_argument("--beam-option-cap", type=int, default=300)
    ...
    result = {
        name: search(
            ...
            beam_jobs=args.beam_jobs,
            beam_option_cap=args.beam_option_cap,
    ...
    out_path = OUT / (
        "five_tile_estimate.json"
        if args.estimate_only
        else "five_tile_certify_search.json"
        if args.certify_from_initial
        else "five_tile_beam_search.json"
        if args.beam_only
        else "five_tile_seed_search.json"
        if args.seed_only or args.beam_only
        else "five_tile_search.json"
    )
